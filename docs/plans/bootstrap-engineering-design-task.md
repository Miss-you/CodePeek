...
| --- | --- | --- | --- | --- | --- | --- | --- | --- | --- | --- | --- |
| T01 | Bootstrap package.json + scripts contract | 落地 `package.json`，定义 `dev` / `build` / `test` / `test:e2e` / `lint` / `typecheck` / `format` / `dist:mac` / `dist:mac:dir` 脚本 | — | — | done | trae-agent | 2026-05-07T00:00:00Z | `workspace/T01/` | `bootstrap-package-scripts`（已归档为 `2026-05-07-bootstrap-package-scripts`） | 仓库根存在 `package.json`；`npm run lint` / `typecheck` / `test` / `build` 四个命令在 M1 完成后能被无错调用（命令存在性即可，本任务无需绿） | 首个验证：`npm run` 列出全部目标脚本 ✅（npm 11 下 `-s` 会静默输出，故统一用 `npm run`）；worktree：`../CodePeek-T01` 分支 `task/T01-bootstrap-package-scripts` |
...
- `2026-05-07T00:05:00Z` · T01 · `todo` → `claimed`，owner=trae-agent；隔离 worktree `../CodePeek-T01` 分支 `task/T01-bootstrap-package-scripts` 已就绪；workspace `workspace/T01/` 已创建；OpenSpec change `bootstrap-package-scripts` 在 spec 阶段创建。
- `2026-05-07T01:00:00Z` · T01 · `claimed` → `spec`：完成 `current_state.md` / `reference_impl.md` / `final_impl_v1.md` / `final_impl.md`（rubric 88.6 分通过）；创建 OpenSpec change `bootstrap-package-scripts`（proposal / design / specs / tasks）并通过 `openspec validate --strict`；产出 `test_strategy.md`。
- `2026-05-07T02:00:00Z` · T01 · `spec` → `implementing` → `verifying`：落地根 `package.json` 含 9 条核心 + 3 条辅助脚本；`npm install --ignore-scripts`（Electron 二进制 postinstall 因 ECONNRESET 跳过，已入 todo.md）生成 `package-lock.json`；`npm run` 列出 12 条脚本；4 个关键脚本 dry run 均无 `missing script`。
- `2026-05-07T03:00:00Z` · T01 · `verifying` → `review`：自查三视角（契约消费者 / 可维护性 / 参考忠实度）无高严重度问题；中度遗留已分流到 `workspace/T01/todo.md` #1–#6。
- `2026-05-07T04:00:00Z` · T01 · `review` → `done`：final compare 无未记录偏差；OpenSpec change 归档为 `2026-05-07-bootstrap-package-scripts`；capability `package-scripts-contract` 已写入 `openspec/specs/`。T02–T09 解锁。
